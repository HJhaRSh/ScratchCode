...
export default function CelebrationModal({ isOpen, onClose, resultData, questData, onViewLeaderboard }: CelebrationModalProps) {
  ...
  const aiApproach = aiFeedback?.betterApproach?.exists && aiFeedback?.betterApproach?.codeSnippet ? [{
    name: aiFeedback.betterApproach.name || 'AI Suggested Approach',
    timeComplexity: aiFeedback.betterApproach.timeComplexity || 'O(?)',
    spaceComplexity: aiFeedback.betterApproach.spaceComplexity || 'O(?)',
    explanation: aiFeedback.betterApproach.whyBetter || 'A more optimal approach generated by AI.',
    code: aiFeedback.betterApproach.codeSnippet || ''
  }] : [];

  const allApproaches = [...(solutionApproaches || []), ...aiApproach];
  const hasApproaches = allApproaches.length > 0;
          ...
          >
            ...
            <div className="flex-1 overflow-hidden flex flex-col min-h-0">
              ...
              <div className="flex-1 overflow-y-auto p-6 custom-scrollbar bg-black/20">
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="grid md:grid-cols-2 gap-4">
                      <div className="bg-emerald-500/5 border border-emerald-500/10 rounded-xl p-4">
                        <h4 className="text-emerald-500 font-bold mb-3 flex items-center gap-2"><CheckCircle2 className="h-4 w-4" /> Strengths</h4>
                        <ul className="text-sm text-slate-300 space-y-2 list-disc ml-5">
                          {aiFeedback.codeQuality.strengths.map((s: string, i: number) => <li key={i} className="leading-relaxed">{s}</li>)}
                        </ul>
                      </div>
                      <div className="bg-rose-500/5 border border-rose-500/10 rounded-xl p-4">
                        <h4 className="text-rose-500 font-bold mb-3 flex items-center gap-2"><ChevronRight className="h-4 w-4" /> Areas to Improve</h4>
                        <ul className="text-sm text-slate-300 space-y-2 list-disc ml-5">
                          {aiFeedback.codeQuality.improvements.map((s: string, i: number) => <li key={i} className="leading-relaxed">{s}</li>)}
                        </ul>
                      </div>
                    </div>
                ...
                {activeTab === 'approaches' && hasApproaches && (
                  <div className="space-y-8">
                    {allApproaches.map((app: any, idx: number) => {
